class Grade2Controller < ApplicationController
  add_breadcrumb 'Home', '/'
 ...
 def index
  ...
  def tangen
    ...
    add_breadcrumb '中学2年生', '/grade2/index'
    add_breadcrumb @grade_unit_item_name

    @grade_lessons = Lesson.where(["unit_item_name = ? and grade = ?", @grade_unit_item_name, 2]).select('id, title, summary, exercise, point')
  ...
  def show
    ...
    add_breadcrumb '中学2年生', '/grade2/index'
    add_breadcrumb @grade_unit_item_name,'/grade2/'+@grade_unit_item_name
    ...
    @title = '中学2年生数学の' + @lesson
    @description = '中学2年生で勉強する数学の単元「' + @grade_unit_item_name + '」の「'+ @lesson +'」内容です。'
